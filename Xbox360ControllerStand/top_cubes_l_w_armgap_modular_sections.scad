...
section_sequence = [0,1,2,3];           // which templates to place in each slot (can repeat)
section_show     = [true,true,true,true]; // per-slot visibility (same length as section_sequence)
...
module sections() {
  for (slot = [0 : len(section_sequence)-1]) {
    if (slot < len(section_show) && section_show[slot]) {
      place_section(slot, section_sequence[slot]);
    }
  }
}
