...
depth       = 22.00;          // front-to-back thickness
...
knob_offset = 2.00;    // knob starts this far above bottom edge
...
module knobs() {
    ...
    // Left knob (points +X)
    translate([-(inner_gap/2) + knob_len/2, 0, knob_z])
        rotate([0,90,0])
            cylinder(d=knob_diam, h=knob_len, center=true);
